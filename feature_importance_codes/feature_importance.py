...
def loop_through_all(chosen_model, label, dataset, foodfeatures):
    ...
    if dataset=="Labeled":
        ...
        if label=="Prediction":
            if chosen_model=="XGBoost":
                ...
                data = pd.read_csv(r"D:\Vittoria\Code\data\correlation\labeled_features_no_correlation_RandomForest_Prediction.csv")
                feature_columns = [col for col in data.columns if col not in ['Time', 'PersStatus', 'Glucose_Mean', 'DiabStatus', 'ID']]
                if foodfeatures=="FoodNo":
                    feature_columns = feature_columns[:26] + feature_columns[31:]
        elif label=="Classification":
            if chosen_model=="XGBoost":
                data = pd.read_csv(r"D:\Vittoria\Code\data\correlation\labeled_features_no_correlation_XGBoost_Classification.csv")
                feature_columns = [col for col in data.columns if col not in ['Time', 'PersStatus', 'Glucose_Mean', 'DiabStatus', 'ID']]
                if foodfeatures=="FoodNo":
                    feature_columns = feature_columns[:25] + feature_columns[31:]
            elif chosen_model=="RandomForest":
                data = pd.read_csv(r"D:\Vittoria\Code\data\correlation\labeled_features_no_correlation_RandomForest_Classification.csv")
                feature_columns = [col for col in data.columns if col not in ['Time', 'PersStatus', 'Glucose_Mean', 'DiabStatus', 'ID']]
                if foodfeatures=="FoodNo":
    ...
    #if dataset == "Labeled":
        #sorted_df.to_csv(rf"D:\Vittoria\Code\data\feature_importance\sorted_importance_{chosen_model}_{label}_{dataset}_{foodfeatures}.csv", index=False)
