...
# Set random seed and deterministic mode for reproducibility
pl.seed_everything(42)
torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# Load feature and demographic data for prediction task
...
# Create a time index per participant based on minutes from the start of their time series
...
# Ensure participant IDs are strings (for categorical encoding)
data["Participant_ID"] = data["Participant_ID"].astype(str)

# Drop time column as it's no longer needed
data = data.drop(columns=["Time"])

# Define sequence lengths for encoder and prediction
...
# Loop over participants, leaving one out each time for evaluation
for test_id in participant_ids:

    train_data = data[data["Participant_ID"] != test_id]
    test_data = data[data["Participant_ID"] == test_id]
    test_data["Participant_ID"] = test_data["Participant_ID"].astype(str)

    if __name__ == '__main__' and test_id != 2:    
        training = TimeSeriesDataSet(
            ...
            time_varying_known_reals=["time_idx","Hours_from_midnight", "Minutes_from_midnight", "HRV_Max", "HRV_Min", "HRV_Mean", "HRV_Median", "SDNN", "RMSSD", "NNX", "PNNX", 
                        ...
                        "TEMP_q1G", "TEMP_q3G", "TEMP_skew", "ACC_Mean", "ACC_Std", "ACC_Min", "ACC_Max", 
                        "ACC_q1G", "ACC_q3G", "ACC_skew", "ACC_Mean2hrs", "ACC_Max2hrs", 
                        "Activity Bouts","Activity Bouts_24hr","Activity Bouts_1hr","Wake_Time"],
        ...
        # Dataloaders and training setup
        batch_size = 32
        ...
        # Define TFT model with specific hyperparameters
...
# Convert metric lists to numpy arrays and print aggregated results
...
# Generate raw prediction outputs for plotting
...
# Save forecast plots for each prediction sample
for idx in range(len(prediction_output[0].detach().cpu().numpy())):
    ...
    fig.savefig(rf"D:\Vittoria\Code\torch_fc\other\forecast_sample_{idx+1}_nofood.png", dpi=300, bbox_inches="tight")
    plt.close(fig)