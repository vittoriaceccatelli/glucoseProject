...
if dataset=="BalancedRandom":
    if correlation=="Labeled":
        data = pd.read_csv(r"D:\Vittoria\Code\data\balanced_datasets\balanced_dataset_random_Labeled.csv")
        ...
        data = pd.read_csv(r"D:\Vittoria\Code\data\balanced_datasets\balanced_dataset_random_NoCorrelation.csv")
        demo = pd.read_csv(r"D:\Vittoria\Code\data\other\labeled_demographics.csv")
        data = pd.merge(data, demo, left_on="Participant_ID", right_on="ID")
        data['Gender'] = data['Gender'].map({'MALE': 0, 'FEMALE': 1})
        feature_columns = [col for col in data.columns if col not in ['PersStatus', 'ID', 'Glucose_Mean', 'Time', 'DiabStatus']]
    ...
    if correlation=="Labeled":
        data = pd.read_csv(r"D:\Vittoria\Code\data\balanced_datasets\balanced_dataset_more_continuous_Labeled.csv")
        demo = pd.read_csv(r"D:\Vittoria\Code\data\other\labeled_demographics.csv")
        data = pd.merge(data, demo, left_on="Participant_ID", right_on="ID")
        data['Gender'] = data['Gender'].map({'MALE': 0, 'FEMALE': 1})
        feature_columns = [col for col in data.columns if col not in ['PersStatus', 'ID', 'Glucose_Mean', 'Time', 'DiabStatus']]
        ...
        data = pd.read_csv(r"D:\Vittoria\Code\data\balanced_datasets\balanced_dataset_more_continuous_NoCorrelation.csv")
        data['Gender'] = data['Gender'].map({'MALE': 0, 'FEMALE': 1})
        feature_columns = [col for col in data.columns if col not in ['PersStatus', 'ID', 'Glucose_Mean', 'Time', 'DiabStatus']]
        if food=="foodNo":
            #drop food log columns
    ...
    if correlation=="Labeled":
        ...
        data = pd.read_csv(r'D:\Vittoria\Code\data\correlation\labeled_features_no_correlation_XGBoost_Prediction.csv')
        #get names of feature columns and divide data into features and labels and get group IDs (participant IDs)
        feature_columns = [col for col in data.columns if col not in ['PersStatus', 'ID', 'Glucose_Mean', 'Time', 'DiabStatus']]
        if food=="foodNo":
            #drop food log columns
...
for participant_id in groups.unique():
    ...
    results.extend([
        {"Prediction": y_pred[i], "True": y_test[i]}
...
results_df.to_csv(rf"D:\Vittoria\Code\ba-plots\predictions\personalised_nofood.csv")
...
for train_idx, test_idx in logo.split(X, y, groups):
    ...
    results.extend([
        {"Prediction": y_pred[i], "True": y_test.values[i]}
        for i in range(len(y_pred))
    ])
...
results_df = pd.DataFrame(results)
results_df.to_csv(rf"D:\Vittoria\Code\ba-plots\predictions\population_nofood.csv")